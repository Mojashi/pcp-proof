theory aut2_conf_step_1_011
  imports Main "PCPLib.PCPAutomata" "aut2_conf" "aut19_conf" "pcp_instance" "aut18_pref_quotient_1" "aut2_append_word_011"
begin

lemma pref_quotient_lemma:
  "lang aut19 = lang (pref_quotient (append_word aut2 tile_2_dn) tile_2_up)"
proof -
  have "lang (append_word aut2 tile_2_dn) = lang aut18"
    by (simp only: aut2_append_word_011 tile_2_dn_def )

  then have A:"lang (pref_quotient (append_word aut2 tile_2_dn) tile_2_up) = lang (pref_quotient aut18 tile_2_up)"
    by (simp only: pref_quotient_hom[of "append_word aut2 tile_2_dn" aut18] )

  have "lang (pref_quotient aut18 tile_2_up) = lang aut19"
    by (simp only: aut18_pref_quotient_1 tile_2_up_def )
  then show ?thesis using A by simp
qed


lemma is_stepped_autconf:
  "lang_autconf aut19_conf = lang_autconf (fst (step_autconf_tile aut2_conf tile_2))"
  apply (simp only: aut19_conf_def aut2_conf_def tile_2_def)
