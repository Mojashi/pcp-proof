theory aut1_conf_step_1_1110
  imports Main "PCPLib.PCPAutomata" "aut1_conf" "aut10_conf" "pcp_instance" "aut3_pref_quotient_1110" "aut1_append_word_1"
begin
...
proof -
  ...
  have "lang (pref_quotient aut3 tile_2_dn) = lang aut10"
    by (simp only: aut3_pref_quotient_1110 tile_2_dn_def )
...
proof -
   ...
   have A1:"(Set.filter (\<lambda>(s,p). accept' aut1 s \<and> starts_with p tile_2_up) (enumerate_splits tile_2_dn)) = {([C1], [C1,C1,C0]), ([C1,C1], [C1,C0])}"
     apply (simp only: tile_2_dn_def)
     apply auto
     apply (simp_all only: aut1_def starts_with_def tile_2_up_def)
     by auto

   have A2:"(\<lambda> (s,p). (drop (length tile_2_up) p)) ` ... = {[C1,C0], [C0]}"
     apply (simp only: tile_2_up_def)
     by auto

   have "(\<lambda> (s,p). (drop (length tile_2_up) p)) ` (Set.filter (\<lambda>(s,p). accept' aut1 s \<and> starts_with p tile_2_up) (enumerate_splits tile_2_dn)) = {[C1,C0], [C0]}"
     by (simp only:A1 A2)
