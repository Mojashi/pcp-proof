theory aut2_conf_step_10_11
  imports Main "PCPLib.PCPAutomata" "aut2_conf" "aut14_conf" "pcp_instance" "aut13_pref_quotient_10" "aut2_append_word_11"
begin

lemma pref_quotient_lemma:
  "lang aut14 = lang (pref_quotient (append_word aut2 tile_1_dn) tile_1_up)"
proof -
  have "lang (append_word aut2 tile_1_dn) = lang aut13"
    by (simp only: aut2_append_word_11 tile_1_dn_def )

  then have A:"lang (pref_quotient (append_word aut2 tile_1_dn) tile_1_up) = lang (pref_quotient aut13 tile_1_up)"
    by (simp only: pref_quotient_hom[of "append_word aut2 tile_1_dn" aut13] )

  have "lang (pref_quotient aut13 tile_1_up) = lang aut14"
    by (simp only: aut13_pref_quotient_10 tile_1_up_def )
  then show ?thesis using A by simp
qed


lemma is_stepped_autconf:
  "lang_autconf aut14_conf = lang_autconf (fst (step_autconf_tile aut2_conf tile_1))"
  apply (simp only: aut14_conf_def aut2_conf_def tile_1_def)
