theory aut2_conf_step_1_1110
  imports Main "PCPLib.PCPAutomata" "aut2_conf" "aut16_conf" "pcp_instance" "aut15_pref_quotient_1" "aut2_append_word_1110"
begin

lemma pref_quotient_lemma:
  "lang aut16 = lang (pref_quotient (append_word aut2 tile_1_dn) tile_1_up)"
proof -
  have "lang (append_word aut2 tile_1_dn) = lang aut15"
    by (simp only: aut2_append_word_1110 tile_1_dn_def )

  then have A:"lang (pref_quotient (append_word aut2 tile_1_dn) tile_1_up) = lang (pref_quotient aut15 tile_1_up)"
    by (simp only: pref_quotient_hom[of "append_word aut2 tile_1_dn" aut15] )

  have "lang (pref_quotient aut15 tile_1_up) = lang aut16"
    by (simp only: aut15_pref_quotient_1 tile_1_up_def )
  then show ?thesis using A by simp
qed


lemma is_stepped_autconf:
  "lang_autconf aut16_conf = lang_autconf (fst (step_autconf_tile aut2_conf tile_1))"
  apply (simp only: aut16_conf_def aut2_conf_def tile_1_def)
