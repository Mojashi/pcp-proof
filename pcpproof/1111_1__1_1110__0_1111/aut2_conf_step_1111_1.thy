...
definition stepped_confs::"PCPTrans.config set" where
  "stepped_confs \<equiv> {(PCPTrans.UP [C1,C1])}"
...
proof -
   ...
   have A1:"(Set.filter (\<lambda>(s,p). accept' aut2 s \<and> starts_with p tile_0_dn) (enumerate_splits_all tile_0_up)) = {([C1], [C1,C1,C1])}"
     apply (simp only: tile_0_up_def)
     apply auto
     apply (simp_all only: aut2_def starts_with_def tile_0_dn_def)
     by auto

   have A2:"(\<lambda> (s,p). (drop (length tile_0_dn) p)) ` ... = {[C1,C1]}"
     apply (simp only: tile_0_dn_def)
     by auto

   have "(\<lambda> (s,p). (drop (length tile_0_dn) p)) ` (Set.filter (\<lambda>(s,p). accept' aut2 s \<and> starts_with p tile_0_dn) (enumerate_splits_all tile_0_up)) = {[C1,C1]}"
     by (simp only:A1 A2)
