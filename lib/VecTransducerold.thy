...
qed 
term calc_output_from_euc_list
lemma calc_output_from_euc_add_1':
  assumes "distinct (new_edge#es')"
  shows "calc_output_from_euc_list (new_edge#es') (old(new_edge:=old new_edge + 1)) =
         add_lincomb (calc_output_from_euc_list (new_edge#es') old) (none_then_zero (out new_edge))"
proof -
  have "calc_output_from_euc_list (new_edge#es') (old(new_edge:=old new_edge + 1)) =
        add_lincomb (calc_output_from_euc_list es' (old(new_edge:=old new_edge + 1)))
